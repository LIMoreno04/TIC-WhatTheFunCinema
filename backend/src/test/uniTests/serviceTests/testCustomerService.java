...
class testCustomerService {
        ...
        void testAddCard_NewCard_Success() throws InvalidDataException {
                ...
                Mockito.when(cardRepo.existsByCardNumberAndCardTypeAndExpirationDateAndCvvAndHolderName(
                                cardNumber, "VISA", expirationDate.toString(), "123", "Card Holder")).thenReturn(false);
                Mockito.when(cardRepo.save(Mockito.any(Card.class)))
        ...
        void cancelReservation_ValidData_CancelsReservation() throws InvalidDataException {
                ...
                when(reservationRepo.findByScreeningAndColAndRow(screening, col, row))
                ...
                assertFalse(customer.getReservations().contains(reservation),
                                "La reserva debería haber sido eliminada de las reservas del cliente.");
                assertFalse(screening.getReservations().contains(reservation),
                                "La reserva debería haber sido eliminada de las reservas de la proyección.");
        }
